...
func init() {
	...
	config.FgECRAccessKey = f.String("fg-ecr-access-key", "", "configure the aws key to access ecr")
	config.FgECRRegion = f.String("fg-ecr-region", "", "configure aws region for ecr")
	config.FgECRRegistryID = f.String("fg-ecr-registry-id", "", "configure aws ecr registry id to use, default will be account default")
	config.FgECRSecret = f.String("fg-ecr-secret", "", "configure aws secret for ecr")
	...
	config.FgHelmRepoUrl = f.String("fg-helm-repo-url", "", "helm repo url to upload the final packaged chart")
